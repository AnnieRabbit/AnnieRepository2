...
function modify(element){
	 ...
	 $.get(submit_url,function(data){	
		$("#fid").val(data.first_id);
		$("#id").val(data.thrid_id);
		$("#classname").val(data.thrid_classname);
		$("#seq").val(data.thrid_seq).prop("style","height: 35px; vertical-align: middle; display:inline;")
		var content='';
	    content+="<option value='"+data.second_id+"'>"
		+data.second_classname+"</option>";
			...
			/*alert("成功！")*/
				...
				var	tbody_content="";
					$(pager.datas).each(function(index,item){								
						...
						'<tr>'+		
								...
								'<td>'+item.thrid_addtime+'</td>'+
								'<td>'+item.thrid_seq+'</td>'+	
								'<td>'+		
								'<button type="button"  class="btn btn-success"   onclick="modify(this)" submit_url="'+short_url+'/thrid/'+item.thrid_id+'" short_url="'+short_url+'"  > 编辑 </button>'+
								'&nbsp;'+
								'<button  type="button"   class="btn btn-success" onclick="r(this)"      submit_url="'+short_url+'/thrid/'+item.thrid_id+'" short_url="'+short_url+'"> 删除 </button>'+
								'</td>'+		
						'</tr>;';		
				...
				var tfoot_content="";
				    tfoot_content+=
							...
							'&nbsp;&nbsp;&nbsp;&nbsp;'+
							'<button type="button" class="btn btn-outline-danger btn-round waves-effect waves-light m-1" onclick="split_submit(this)" submit_url="'+short_url+'es/thrid/'+pager.first+'") >First </button>'+
							'<button type="button" class="btn btn-outline-success btn-round waves-effect waves-light m-1" onclick="split_submit(this)" submit_url="'+short_url+'es/thrid/'+pager.pre+'") >Previous </button>'+
							'<button type="button" class="btn btn-outline-info btn-round waves-effect waves-light m-1" onclick="split_submit(this)" submit_url="'+short_url+'es/thrid/'+pager.next+'")  >Next</button>'+
							'<button type="button" class="btn btn-outline-warning btn-round waves-effect waves-light m-1" onclick="split_submit(this)" submit_url="'+short_url+'es/thrid/'+pager.last+'") >Last page</button>'+
							'</th>'+
						'</tr>;';  
